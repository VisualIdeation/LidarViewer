...
int main(int argc,char* argv[])
	...
	std::string tempOctreeFileNameTemplate="/tmp/LidarSubtratorTempOctree";
	unsigned int maxNumPointsPerNode=4096;
	std::string tempPointFileNameTemplate="/tmp/LidarSubtractorTempPoints";
	
	try
		{
		/* Open LidarViewer's configuration file: */
		Misc::ConfigurationFile configFile(LIDARVIEWER_CONFIGFILENAME);
		Misc::ConfigurationFileSection cfg=configFile.getSection("/LidarSubtractor");
	...
	/* Construct an octree with less than maxNumPointsPerNode points per leaf: */
