...
void loadPointFileLasClassify(PointAccumulator& pa, const char* fileName,
	const float colorMask[3], const double pointOffset[3]) {

	//Should check to see if this file exists before opening it
	Threads::GzippedFileCharacterSource class_file("classification.cfg");
	Misc::ValueSource reader(class_file);
	reader.setPunctuation('#', true);
	reader.setPunctuation('\n', true);
	reader.skipWs();

	const int colorsSize = 32;
	struct color {
	int r;
	int g;
	int b;
	...
	while (!reader.eof()) {
		...
		if (reader.peekc() != '#' && reader.peekc() != '\n') {
			try {
				int readClassNum = int(reader.readNumber());
				int readRedNum = int(reader.readNumber());
				int readGreenNum = int(reader.readNumber());
				int readBlueNum = int(reader.readNumber());
				if (readClassNum >= 0 && readClassNum < 32) {
					if (readRedNum >=0 && readRedNum < 256)
						colors[readClassNum].r = readRedNum;
					if (readGreenNum >=0 && readGreenNum < 256)
						colors[readClassNum].g = readGreenNum;
					if (readBlueNum >=0 && readBlueNum < 256)
						colors[readClassNum].b = readBlueNum;
				}
			} catch (Misc::ValueSource::NumberError err) {
				std::cerr << "loadPointFileLasClassify: Classification parsing error in line "
						<< lineNumber << std::endl;
			}
		}
	...
	file.read<unsigned short> (); // Ignore file creation year
	file.read<unsigned short> (); // Ignore header size
	Misc::LargeFile::Offset pointDataOffset = Misc::LargeFile::Offset(
			file.read<unsigned int> ());
	file.read<unsigned int> (); // Ignore number of variable-length records
	unsigned char pointDataFormat = file.read<unsigned char> ();
	unsigned short pointDataRecordLength = file.read<unsigned short> ();
	size_t numPointRecords = file.read<unsigned int> ();
	...
	/* Check the LAS file header for sanity: */
	std::cout << "Point Data Format: " << pointDataFormat << ", " << short(
			pointDataFormat) << std::endl;
	switch (pointDataFormat) {
	case 0:
		if (pointDataRecordLength != 20)
			return;
	...
	for (size_t i = 0; i < numPointRecords; ++i) {
		...
		/* Skip irrelevant information: */
		unsigned char returnInformation = file.read<unsigned char> ();
		short classification = short(file.read<unsigned char> ());
		char scanAngle = file.read<char> ();
		unsigned char userData = file.read<unsigned char> ();
		unsigned short pointSourceID = file.read<unsigned short> ();
		if (pointDataFormat & 0x1)
			double GPSTime = file.read<double> ();
		if (pointDataFormat >= 2) {
			/* Read RGB data: */
			unsigned short rgb[3];
			file.read(rgb, 3);
		}
		/* Assign point color from classification */
			p.value[0] = Color::Scalar(colors[classification].r);
			p.value[1] = Color::Scalar(colors[classification].g);
			p.value[2] = Color::Scalar(colors[classification].b);
			p.value[3] = Color::Scalar(255);
...
void loadPointFileLasIntense(PointAccumulator& pa, const char* fileName,
		const float colorMask[3], const double pointOffset[3]) {
	/* Open the LAS input file: */
	Misc::LargeFile file(fileName, "rb", Misc::LargeFile::LittleEndian);
	...
	for(int i=1;i<argc;++i)
		...
		if(argv[i][0]=='-')
			...
			if(strcasecmp(argv[i]+1,"o")==0)
				...
				pointFileType=LAS;
			else if (strcasecmp(argv[i] + 1, "classify") == 0)
				classify = true;
			else if (strcasecmp(argv[i] + 1, "intense") == 0)
				intense = true;
			...
			switch(thisPointFileType)
				...
				case LAS:
				std::cout << "Processing binary input file " << argv[i]
						<< "..." << std::flush;
				if (classify)
					loadPointFileLasClassify(pa, argv[i], colorMask, lasOffset);
				else if (intense)
					loadPointFileLasIntense(pa, argv[i], colorMask, lasOffset);
				else
